...
def detect(imgframe, confthread=0.25, iouthread=0.35, autonms=True, classes=None,
           imgsz=640, device=None, model=None):
    # Initialize
    set_logging()
    ...
    return list(zip(z_box, picked_score, z_cntr))



if __name__ == '__main__':

    #images = glob.glob('G:/ozone_live/Person_data/Frames/cam_01/*.jpg')
    cap = cv2.VideoCapture('G:/ozone_live/video_data/00000000105000000.mp4')
    path_save_person = 'G:/ozone_live/Person_data/persons/cam_1501/'
    #print(len(images))
    device = select_device('0')
    ...
    while(cap.isOpened()):
        ...
        prddata = detect(ss,confthread=0.6,device=device,model=model)
        if count_images%3 == 0:
            for pp, ii, nn in prddata:
